...
const toolDisplayName: Record<string, string> = {
  str_replace_editor: "Code Editor",
                      ...
                      {message.parts.map((part, partIndex) => {
                        switch (part.type) {
                          ...
                          case "tool-invocation":
                            ...
                            return (
                              <div key={partIndex} className="inline-flex items-center gap-2 mt-2 px-3 py-1.5 rounded-lg text-xs font-mono text-white" style={{ background: "linear-gradient(45deg, #f09433, #e6683c, #dc2743, #cc2366, #bc1888)" }}>
                                {tool.state === "result" && tool.result ? (
                                  <>
                                    <div className="w-2 h-2 rounded-full bg-white/80"></div>
                                    <span className="text-white">{toolDisplayName[tool.toolName] ?? tool.toolName}</span>
                                  </>
                                ) : (
                                  <>
